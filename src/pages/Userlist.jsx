...
import { Button, ButtonGroup, DropdownButton, Form, InputGroup } from 'react-bootstrap';
import { EyeIcon, TrashIcon, SimpleDotedIcon, CancelIcon, DownloadIcon, CheckBoxIcon, SearchIcon, EditICon, DisabledIcon } from '../components/SVGIcon';
...
function Userlist(props) {
  ...
  const modalToggle = async (e, id) =>{
    ...
    jwtAxios.get(`/users/viewUser/${id}`)
    .then((res) => {
      setViewKYC(res?.data);
        ...
        if (id) {
          jwtAxios
            ...
            .then((res) => {
              ...
              set(ref(database, firebaseMessages.ICO_USERS + '/'+ id+'/is_active'),true);
            })
            .catch((err) => {
        ...
        if (id) {
          jwtAxios
            ...
            .then((res) => {
              ...
              set(ref(database, firebaseMessages.ICO_USERS + '/'+ id + '/is_active'),false);
            })
            .catch((err) => {
              if(typeof err == "string")
              {
        ...
        if (id) {
          jwtAxios
            ...
            .then((res) => {
              ...
              Swal.fire(
                "Disabled!",
                "This user's google 2FA disabled.",
                "danger"
              );
            })
            .catch((err) => {
        ...
        if (id) {
          jwtAxios
            ...
            .then((res) => {
              ...
              set(ref(database, firebaseMessages.ICO_USERS + '/'+ id + '/is_delete'), true);
            })
            .catch((err) => {
              if(typeof err == "string")
              {
  ...
  return (
    <div className="transaction-view">
        ...
        <div className="flex-table-top">
          ...
          {users?.map((item, index) => {
            if(item?.last_login)
            {
              ...
              var lastLoginTime = lastLogin.split(" ")[1]+" "+lastLogin.split(" ")[2];
            }
            return (
      ...
      <EditUserDetails show={editModalShow} viewKYC={viewKYC} getusers={getusers} onHide={() => setEditModalShow(false)} />
    </div>
  );
}
