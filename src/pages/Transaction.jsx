...
import { Button, ButtonGroup, Dropdown, DropdownButton, Form, InputGroup, Pagination } from 'react-bootstrap';
import { CheckCircleIcon, CloseIcon, ExclamationIcon, EyeIcon, SettingIcon, TrashIcon, SimpleDotedIcon, CheckBoxIcon, CancelIcon, SearchIcon } from '../components/SVGIcon';
...
function Transaction() {
  ...
  }, [debouncedSearchValue, statusFilter, isTypeChecked, isStatusChecked]);
        ...
        <div className="flex-table">
          <div className="flex-table-top">
            <ButtonGroup aria-label="filter-btn">
              ...
              <Button onClick={() => changeStatus("new")}
                  className={
                    statusFilter == "new" ? "active" : ""
                  } 
              variant="outline-secondary">New</Button>
              <Button onClick={() => changeStatus("pending")}
                  className={
                    statusFilter == "pending" ? "active" : ""
                  } 
              variant="outline-secondary">Pending</Button>
              <Button onClick={() => changeStatus("paid")}
                  className={
                    statusFilter == "paid" ? "active" : ""
                  } 
              variant="outline-secondary">Paid</Button>
              <Button onClick={() => changeStatus("expired")}
                  className={
                    statusFilter == "expired" ? "active" : ""
                  } 
              variant="outline-secondary">Expired</Button>
          ...
          <div className="flex-table-body" key={transaction?._id}>
            <div className="transaction-tranx-no">
              ...
              <div>
                ...
                <p>{getDateFormate(transaction?.created_at,"MMM DD, YYYY HH:mm:ss")}</p>
              </div>
            </div>
            <div className="transaction-token">
              <p className="text-white mb-1">{transaction?.token_cryptoAmount <= 200 ? formattedNumber(transaction?.token_cryptoAmount) : "+200"}</p>
              <p>Token</p>
            </div>
            <div className="transaction-amount">
              <p className="text-white mb-1">{formattedNumber(transaction?.price_amount)}</p>
              <p>{transaction?.price_currency}</p>
            </div>
            <div className="transaction-usd-amount">
              <p className="text-white mb-1">{formattedNumber(transaction?.usd_amount ? transaction?.usd_amount : transaction?.price_amount)}</p>
              <p>{transaction?.receive_currency}</p>
            </div>
            <div className="transaction-from">
              <p className="text-white mb-1">Pay via {transaction?.price_currency} </p>
              <p>{getDateFormate(transaction?.created_at,"MMM DD, YYYY HH:mm:ss")}</p> 
            </div>
            <div className="transaction-type">
              <div className="d-flex justify-content-between align-items-center">
                <Button variant="outline-success" >{transaction.source ? transaction.source.charAt(0).toUpperCase() + transaction.source.slice(1) : "Purchase"}</Button>
